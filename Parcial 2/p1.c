...
int main(){
    ...
    do {
        ...
        if (i != -1){
            ...
            } else { //peso rechazado debido que excede.
                vRechazado[i] += dato; //se suma la perdida
                printf("\nCamion %d rechazo %d kilos por exceso de peso.", i, vRechazado[i]);
            }
        }
    } while (i != -1);
    printf("\n===================");
    ...
    //c) Informar el Código de vehículo, que ha tenido la mayor cantidad de viajes rechazados por exceso de kilos (si hay varios, informar el último que aparezca).
//empezamos de 1 porque lo ponemos como para comprar primero
